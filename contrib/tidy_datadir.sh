...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Kh69coin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Kh69coin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Kh69coin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Kh69coin 0.7 datadir."
    ;;
  3)
    echo "Detected Kh69coin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Kh69coin 0.8 datadir."
    ;;
esac
