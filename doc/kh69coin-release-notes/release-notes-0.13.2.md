Kh69coin Core version 0.13.2 is now available from:

  <https://download.kh69coin.org/kh69coin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a kh69coin
...
ECDSA signatures inside Kh69coin transactions now use validation using
...
Previous versions of Kh69coin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Kh69coin Core was to raise the default minimum
relay fee.

Kh69coin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Kh69coin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running kh69coind without having to do any manual configuration.
...
Kh69coin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Kh69coin Core will relay transactions with
...
entirely in Kh69coin Core 0.13, and it is at this time undecided whether the
...
Kh69coin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Kh69coin Core automatically creates a hidden service to listen on, without
manual configuration. Kh69coin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Kh69coin Core is listening, and
...
Kh69coind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Kh69coin
Core to use floating fees. By default, Kh69coin Core will use floating
fees.
...
(default: `0.0002` 69C/kB).

At all times, Kh69coin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) 69C.
Furthermore, Kh69coin Core will never create transactions paying less than
...
`kh69coin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 22442 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 22442 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /kh69coinrpc>
        ProxyPass http://127.0.0.1:2442/
        ProxyPassReverse http://127.0.0.1:2442/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass kh69coind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< kh69coinrpc:<password>
    </Location>
...
Restarting kh69coind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in kh69coin.conf
...
kh69coin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/kh69coin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Kh69coin Core code base has
...
- `kh69coin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `kh69coin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Kh69coin scripting system that in combination with
...
Kh69coin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Kh69coin Core.
...
The two phases are distinct in the Kh69coin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Kh69coin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Kh69coin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Kh69coin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of kh69coins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of kh69coins being spent is exactly
...
- **Increased security for multisig:** Kh69coin addresses (both P2PKH addresses
  ...
  believed to be provided by Kh69coin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Kh69coin's current ledger (called the UTXO set),
  ...
  note that Kh69coin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Kh69coin users to individually opt-in to almost any change in the Kh69coin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Kh69coin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Kh69coin
client to Kh69coin Core 0.13.2 to signal for segwit support. When a
...
Since Kh69coin Core 0.10.0, nodes have defaulted to only relaying and
...
- `kh69coin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `kh69coin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
