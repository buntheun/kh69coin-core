...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/kh69coin-project/kh69coin>";
    const std::string URL_WEBSITE = "<https://kh69coin.org>";
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if kh69coin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if kh69coin is started while in a temporary directory.\n",
